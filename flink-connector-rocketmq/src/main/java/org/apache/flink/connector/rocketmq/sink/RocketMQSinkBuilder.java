...
public class RocketMQSinkBuilder<IN> {
    ...
    private void sanityCheck() {
        checkState(
                StringUtils.isNotBlank(configBuilder.get(RocketMQSinkOptions.ENDPOINTS)),
                "endpoints must be set via setEndpoints()");
        checkState(
                StringUtils.isNotBlank(configBuilder.get(RocketMQSinkOptions.PRODUCER_GROUP)),
                "producer group must be set via setGroupId()");
        checkNotNull(serializer, "serializer must be set via setSerializer()");
    }
