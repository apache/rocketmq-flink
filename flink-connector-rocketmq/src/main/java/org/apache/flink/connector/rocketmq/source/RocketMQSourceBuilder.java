...
public class RocketMQSourceBuilder<OUT> {
    ...
    private void sanityCheck() {
        checkState(
                StringUtils.isNotBlank(configBuilder.get(RocketMQSourceOptions.ENDPOINTS)),
                "endpoints must be set via setEndpoints()");
        checkState(
                StringUtils.isNotBlank(configBuilder.get(RocketMQSourceOptions.CONSUMER_GROUP)),
                "consumer group must be set via setGroupId()");
        checkState(
                StringUtils.isNotBlank(configBuilder.get(RocketMQSourceOptions.TOPIC)),
                "topic must be set via setTopics()");
        checkNotNull(deserializationSchema, "deserialization schema must be set");
    }
