...
/**
 * With Punctuated Watermarks To generate watermarks whenever a certain event indicates that a new watermark might be
 * generated, use AssignerWithPunctuatedWatermarks. For this class Flink will first call the extractTimestamp(...)
 * method to assign the element a timestamp, and then immediately call the checkAndGetNextWatermark(...) method on that
 * element.
 *
 * <p>The checkAndGetNextWatermark(...) method is passed the timestamp that was assigned in the
 * extractTimestamp(...) method, and can decide whether it wants to generate a watermark. Whenever the
 * checkAndGetNextWatermark(...) method returns a non-null watermark, and that watermark is larger than the latest
 * previous watermark, that new watermark will be emitted.
 */
